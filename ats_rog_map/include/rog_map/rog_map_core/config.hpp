...
#define CMAKE_ROOT_DIR(name) (string(string(ROOT_DIR) + name))
    ...
    public:
        ...
        Config(const string &cfg_path, const string &name_space = "rog_map") {
            ...
            loader.LoadParam(
                name_space + "/esdf/update_interval_updates", esdf_update_interval_updates, 1);
            esdf_update_interval_updates = std::max(1, esdf_update_interval_updates);
            ...
            // 概率更新参数，必须满足: p_miss < p_free < 0.5 < p_occ < p_hit
            // p_hit: 射线端点命中时的观测概率，logit(p_hit) > 0 使占据概率升高
            loader.LoadParam(name_space + "/raycasting/p_hit", p_hit, 0.70f);
            // p_miss: 射线穿过(miss)时的观测概率，logit(p_miss) < 0 使占据概率降低
            loader.LoadParam(name_space + "/raycasting/p_miss", p_miss, 0.70f);
            // p_min / p_max: log-odds 的 clamp 范围，防止概率无限累积
            loader.LoadParam(name_space + "/raycasting/p_min", p_min, 0.12f);
            loader.LoadParam(name_space + "/raycasting/p_max", p_max, 0.97f);
            // p_occ: 占据判定阈值，occupancy >= logit(p_occ) 判定为 OCCUPIED
            loader.LoadParam(name_space + "/raycasting/p_occ", p_occ, 0.80f);
            // p_free: 自由判定阈值，occupancy < logit(p_free) 判定为 KNOWN_FREE
            loader.LoadParam(name_space + "/raycasting/p_free", p_free, 0.30f);
            loader.LoadParam(name_space + "/raycasting/p_free", p_free, 0.30f);
            loader.LoadParam(name_space + "/raycasting/stale_decay/enable", stale_decay_en, false);
            loader.LoadParam(name_space + "/raycasting/stale_decay/soft_ttl_updates", stale_soft_ttl_updates, 30);
            loader.LoadParam(name_space + "/raycasting/stale_decay/hard_ttl_updates", stale_hard_ttl_updates, 90);
            loader.LoadParam(name_space + "/raycasting/stale_decay/decay_log_odds_step", stale_decay_log_odds_step, 0.25f);
            loader.LoadParam(name_space + "/raycasting/stale_decay/sweep_interval_updates", stale_sweep_interval_updates, 1);
            loader.LoadParam(name_space + "/raycasting/stale_decay/local_update_box_only",
                             stale_decay_local_update_box_only,
                             true);
            loader.LoadParam(name_space + "/raycasting/clear_clipped_endpoint",
                             clear_clipped_endpoint,
                             true);
            if (stale_soft_ttl_updates < 0) {
                stale_soft_ttl_updates = 0;
            }
            if (stale_hard_ttl_updates < 0) {
                stale_hard_ttl_updates = 0;
            }
            if (stale_hard_ttl_updates > 0 && stale_hard_ttl_updates < stale_soft_ttl_updates) {
                stale_hard_ttl_updates = stale_soft_ttl_updates;
            }
            if (stale_decay_log_odds_step < 0.0F) {
                stale_decay_log_odds_step = -stale_decay_log_odds_step;
            }
            if (stale_sweep_interval_updates <= 0) {
                stale_sweep_interval_updates = 1;
            }
...
#define logit(x) (log((x) / (1 - (x))))
            // 概率 → log-odds 转换: logit(p) = log(p / (1-p))
            // 正值倾向占据，负值倾向自由，0 为中性
            l_hit = logit(p_hit);   // > 0: 每次 hit 增加的 log-odds
            l_miss = logit(p_miss); // < 0: 每次 miss 减少的 log-odds
            l_min = logit(p_min);   // log-ods 下限 clamp
            l_max = logit(p_max);   // log-odds 上限 clamp
            l_occ = logit(p_occ);   // occupancy >= l_occ → OCCUPIED
            l_free = logit(p_free); // occupancy < l_free → KNOWN_FREE

            // n_free: 需要连续 miss 多少次才能从 unknown 降到 free
            // n_occ: 需要连续 hit 多少次才能从 unknown 升到 occupied
        ...
        bool stale_decay_en{false};
        int stale_soft_ttl_updates{30};
        int stale_hard_ttl_updates{90};
        int stale_sweep_interval_updates{1};
        float stale_decay_log_odds_step{0.25f};
